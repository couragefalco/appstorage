...
def step_to_stl(file_path):
    doc = FreeCAD.open(file_path)
    mesh = doc.Objects[0].Shape.tessellate(0.1)
    trimesh_obj = trimesh.Trimesh(vertices=mesh[0], faces=mesh[1])
    trimesh_obj.export(file_path.replace('.step', '.stl'), 'stl')

def preprocess_file(file_path):
    if file_path.endswith('.step'):
        step_to_stl(file_path)
        file_path = file_path.replace('.step', '.stl')
...
def main():
    ...
    database_file = st.file_uploader("Choose a file to add to database", type=['step', 'stl'], key='database_uploader')
